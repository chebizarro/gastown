...
func (d *Daemon) effectiveBeadsBackend() agentconfig.BeadsBackend {
	if d.beadsBackend == "" {
		return agentconfig.BeadsBackendDolt
	}
	return d.beadsBackend
}

func isDoltOnlyPatrol(patrol string) bool {
...
func (d *Daemon) isPatrolActive(patrol string) bool {
	if d.effectiveBeadsBackend() != agentconfig.BeadsBackendDolt && isDoltOnlyPatrol(patrol) {
		return false
	}
